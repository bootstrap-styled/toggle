...
function install(argv, locale, done) {
  ...
  if (packageJson['pre-commit'].indexOf('extract-intl') === -1) {
    ...
    packageJson.scripts['extract-intl'] = `NODE_ENV=production npm run ${binPkgJson.name} -- intl extract${target}`;
  }
  if (packageJson['pre-commit'].indexOf('extract-intl:doc') === -1) {
    packageJson['pre-commit'].push('extract-intl:doc');
    packageJson.scripts['extract-intl:doc'] = `NODE_ENV=production npm run ${binPkgJson.name} -- doc intl`;
  }
